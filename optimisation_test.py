...
from evaluationtest import evaluation
...
def init_indp(icls, ranges, genome=list()):
    ...
    if genome == list():
        ...
        for p in ranges[0:nparam]:
            genome.append(np.random.uniform(*p))#origine random.randint
    return icls(genome)
...
def init_opti():
    ...
    creator.create("FitnessMin", base.Fitness, weights=(-1.0,-1.0)) # weights negative ==> minimize 
...
def parameters(GEN=100, NBIND=96, CX=0.8, MX=0.2):
    """
...
def main():
    ...
    for gen in range(1, NGEN):
        ...
        indiv.extend([ind for ind in pop])
...
def plots(population, plot2D = True, plot3D = False, interactive = False): #optimal_indiv, data
    ...
    if plot2D == True : 
        ...
        plt.scatter(Pareto_objective_functions[:, 0], Pareto_objective_functions[:, 1])
        plt.xlabel("f1")
        plt.ylabel('f2')
    ...
    if interactive == True:
        df = pd.DataFrame({"f1" : Pareto_objective_functions[:, 0], 
                        "f2" :  Pareto_objective_functions[:, 1], 
        ...
        front=df.hvplot.scatter(x='f1', y='f2', hover_cols='x1')
        hvplot.show(front)
...
#        logger.error(e)
    ...
    plots(optimal_front, interactive=True)
